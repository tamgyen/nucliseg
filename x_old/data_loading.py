...
def show(sample, show_masks=True, show_boxes=True):
    ...
    if show_masks:
        for mask in masks:
            ...
            mask = torch.where(mask > 0.2, mask, torch.tensor(0.0))
...
if __name__ == '__main__':
    ds = EndoNukeDataset(root='C:/Dev/Projects/3DHISTEC/01_data/01_dataset_endonuke')

    example = ds[10]

    show(example, show_boxes=False)
